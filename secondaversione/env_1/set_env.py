...
def resume_env(plot=False,
    ...
    geometry_params = {'output': '.'.join([root, 'geo']),
                    ...
                    'set_freq': 1,
                    'set_control':0,
                    'control_terms': ['Qs', 'frequencies', 'control_evolution'],
                    'tuning_parameters' : [4.0,1.0,0.02],
                    'clscale': 1,
                    'template': '../backward_facing_step.template_geo',
                    'remesh': remesh}

    if geometry_params['set_freq']:
        geometry_params['control_terms'] = ['Qs','control_evolution']

    if geometry_params['set_control']:
        geometry_params['control_terms'] = ['Qs','frequencies']
