...
def resume_env(plot=False,
    ...
    geometry_params = {'output': '.'.join([root, 'geo']),
                    ...
                    'set_freq': 0
                    'set_control_length': 0,
                    'control_terms': ['Qs', 'frequencies','control_length'],
                    'tuning_parameters' : [4,1,0.05],
                    'clscale': 1,
                    'template': '../backward_facing_step.template_geo',
                    'remesh': remesh}

    if geometry_params['set_freq']: # If set_freq, you set also control_length
