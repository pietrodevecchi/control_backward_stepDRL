...
class FlowSolver(object):
    ...
    def __init__(self, flow_params, geometry_params, solver_params):    
        ...
        #Provo a iniziare a cambiare Control_Width
        ...
        control_length_base = geometry_params['tuning_parameters'][2]

        #jet_tag = control_tag                                                                                  
        jet = JetBCValue(gtime, length_before_control, control_width, step_height, jet_amplitude_tuning, frequency_amplitude_tuning, control_length_base, frequency=0, Q=0,control_length=0,degree=1)

        #Dynamic Boundary initialization:
        def boundary_control(x, on_boundary,control_length,control_length_base,length_before_control,step_height):
            x_min= length_before_control - 1/20 * control_length
            x_max= control_length_base + length_before_control
            y_height=step_height
            return on_boundary and x[0] >= x_min and x[0] =< x_max and x[1] = y_height

        def boundary_bf_control(x, on_boundary,control_length,control_length_base,length_before_control,step_height):
            x_min = 0
            x_max= length_before_control - 1/20 * control_length
            y_height = step_height
            return on_boundary and x[0] >= x_min and x[0] < x_max and x[1] = y_height 

        #controllo1=boundary_control(x, on_boundary,control_length,control_length_base,length_before_control,step_height)
        #controllo2=boundary_bf_control(x, on_boundary,control_length,control_length_base,length_before_control,step_height)  
        
        # Boundary condition for jet,and the length_before_control here set as no-slip (Cambiato anche per length_bf)
        bcu_jet = DirichletBC(V, jet, boundary_control(x, on_boundary,0,control_length_base,length_before_control,step_height))
        bcu_length_bf_control = DirichletBC(V,Constant(0,0),boundary_bf_control(x, on_boundary,0,control_length_base,length_before_control,step_height))

        # All bcs objects together (where we don't impose anything we have homogeneous Neumann)
        # velocity bcs
        bcu = [bcu_inlet, bcu_wall1, bcu_wall2, bcu_length_bf_control, bcu_jet]
        # pressure bcs
        bcp = [bcp_outflow]
    ...
    def evolve(self, jet_bc_values):
        ...
        # Update BCS
        self.bcu_length_bf_control = DirichletBC(V, jet, boundary_control(x, on_boundary,jet_bc_values [2],control_length_base,length_before_control,step_height))
        self.bcu_jet = DirichletBC(V,Constant(0,0), boundary_control(x, on_boundary,jet_bc_values [2],control_length_base,length_before_control,step_height))
