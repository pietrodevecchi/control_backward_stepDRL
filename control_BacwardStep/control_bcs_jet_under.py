...
class JetBCValue(UserExpression):
    ...
    def eval(self, values, x):
        ...
        # x_ = self.length_before_contr + self.control_width   
        
        y_max = self.control_width

        # evaluation of the profile at point x
        values[0] = self.Q*(x[1]-y_min)*(y_max-x[1])/self.control_width**2 * abs(sin(self.freq*2*pi*self.time))
        values[1] = 0.*x[1]
