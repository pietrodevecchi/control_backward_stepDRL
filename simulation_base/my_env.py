...
from my_Env2DCylinder import Env2DCylinder
import numpy as np
...
def resume_env(plot=False,
    ...
    geometry_params = {'output': '.'.join([root, 'geo']),
                    ...
                    'coarse_distance': 0.5,
                    'box_size': 0.05,
                    'jet_positions': [0.01],    #punto centrale dell'intervallino
                    #'jet_width': 10,
                    'clscale': 0.25,
    ...
    def profile(mesh, degree):
        ...
        return Expression(('-4*Um*(x[1]-bot)*(x[1]-top)/H/H',                              # valore di inflow lo scrive lui
                        '0'), bot=bot, top=top, H=H, Um=Um, degree=degree)                 # degree è il grado di aprossimazione polinomiale
   ...
   #for crrt_radius in list_radius_around:
    ...
    optimization_params = {"num_steps_in_pressure_history": 1,
                        "min_value_jet_MFR": -1.e-1,
                        "max_value_jet_MFR": 1.e-1,
