    ...
    def start_class(self):
       ...
       # buffer initialized to store rewards
        ...
        if self.geometry_params['set_freq']:
            ...
            self.control_length = np.zeros(1)
        else:
            [self.Qs, self.frequencies, self.control_length] = np.zeros(len(self.geometry_params['control_terms']))
        ...
        if self.n_iter_make_ready is not None:
            ...
            # initialize solution
            if self.geometry_params['set_freq']:
                self.u_, self.p_ = self.flow.evolve(np.concatenate((self.Qs, self.frequencies,self.control_length)))
            else:
                self.u_, self.p_ = self.flow.evolve([self.Qs, self.frequencies,self.control_length])     
            path=''
...
# ------------------------------------------------------------------------------------- End of Start_class  ---------------------------------------------------------------
    ...
    def write_history_parameters(self):
        ...
        self.history_parameters["control_for_control_length"].extend(self.control_length)
...
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def sing_run_output(self):
        ...
        if(not os.path.exists("saved_models/"+name)):
            with open("saved_models/"+name, "w") as csv_file:
                spam_writer=csv.writer(csv_file, delimiter=";", lineterminator="\n")
                spam_writer.writerow(["Name", "Step", "RecircArea"] + ["Control_action" + str(v) for v in range(len([self.Qs,self.frequencies,self.control_length]))])
                spam_writer.writerow([self.simu_name, self.solver_step,
                                      self.history_parameters["recirc_area"].get()[-1]] + [str(v) for v in [self.Qs,self.frequencies,self.control_length]])
        else:
            with open("saved_models/"+name, "a") as csv_file:
                spam_writer=csv.writer(csv_file, delimiter=";", lineterminator="\n")
                spam_writer.writerow([self.simu_name, self.solver_step,
        ...
        if "step" in self.inspection_params:
            ...
            if self.solver_step % modulo_base == 0:
                if self.verbose > 0:
                    ...
                    print("Control_variation : {}".format(self.control_length))
        ...
        for crrt_action_nbr in range(self.number_steps_execution):
            ...
            if "smooth_control" in self.optimization_params:
                ...
                if self.geometry_params['set_freq']:
                    ...
                    [self.Qs, self.frequencies,self.control_length] = np.array(self.previous_action) + (np.array(self.action) - np.array(self.previous_action)) / self.number_steps_execution * (crrt_action_nbr + 1)  
                ...
                if self.geometry_params['set_freq']:
                    ...
                    [self.Qs, self.frequencies,self.control_length] = np.transpose(np.array(action))
            ...
            if "zero_net_Qs" in self.optimization_params:
                if self.optimization_params["zero_net_Qs"]:
                    ...
                    self.control_length = self.control_length - np.mean(self.control_length)
