...
class FlowSolver(object):
    ...
    def __init__(self, flow_params, geometry_params, solver_params):    
        ...
        tuning_control_width = geometry_params['tuning_parameters'][2]    #tuning_control_width = b !!!

        self.length_before_control = length_before_control
        self.control_width = control_width
        self.tuning_control_width = tuning_control_width
        self.step_height = step_height
        control_length = 0
        self.control_length = control_length

        #jet_tag = control_tag                                                                                  
        jet = JetBCValue(gtime, length_before_control, control_width, step_height, jet_amplitude_tuning, frequency_amplitude_tuning, tuning_control_width, frequency=0, Q=0,control_length=0,degree=1)

        # Dynamic Boundary initialization:
        def boundary_control(x, on_boundary): #,control_length,tuning_control_width,length_before_control,step_height):
            x_min = self.length_before_control - tuning_control_width * self.control_length
            x_max = self.control_width + self.length_before_control
            y_height= self. step_height
            return on_boundary and x[0] >= x_min and x[0] <= x_max and x[1] == y_height

        def boundary_bf_control(x, on_boundary): #,control_length,tuning_control_width,length_before_control,step_height):
            x_min = 0.0
            x_max= self.length_before_control - tuning_control_width * self.control_length
            y_height = self.step_height
            return on_boundary and x[0] >= x_min and x[0] <= x_max and x[1] == y_height 

        #controllo1=boundary_control(x, on_boundary,control_length,tuning_control_width,length_before_control,step_height)
        #controllo2=boundary_bf_control(x, on_boundary,control_length,tuning_control_width,length_before_control,step_height)  
        ...
        self.bcu_length_bf_control = bcu_length_bf_control
        self.bcu_jet = bcu_jet
        
        # All bcs objects together (where we don't impose anything we have homogeneous Neumann)
        # velocity bcs
        bcu = [bcu_inlet, bcu_wall3, bcu_wall2, self.bcu_length_bf_control, self.bcu_jet]
        # pressure bcs
        bcp = [bcp_outflow]
        ...
        # Assemble matrices
        assemblers = [SystemAssembler(a1, L1, self.bcu),
                      SystemAssembler(a2, L2, bcp),
                      SystemAssembler(a3, L3, self.bcu)]
    ...
    def evolve(self, jet_bc_values):
        ...
        # update control
        self.jet.control_length = jet_bc_values[2]
        self.control_length = jet_bc_values[2]
        ...
        def boundary_bf_control(x, on_boundary): #,control_length,tuning_control_width,length_before_control,step_height):
            ...
            x_max= self.length_before_control - tuning_control_width * self.control_length
            y_height = self.step_height
            return on_boundary and x[0] >= x_min and x[0] <= x_max and x[1] == y_heigh

        # Update BCS
        self.bcu_length_bf_control = DirichletBC(self.V, self.jet, boundary_bf_control) #,jet_bc_values [2],tuning_control_width,length_before_control,step_height))
        self.bcu_jet = DirichletBC(self.V,Constant((0,0)), boundary_control) #,jet_bc_values [2],tuning_control_width,length_before_control,step_height))


        self.bcu[3]=self.bcu_length_bf_control
        self.bcu[4]=self.bcu_jet
    ...
    # def boundary_control(x, on_boundary): #,control_length,tuning_control_width,length_before_control,step_height):
    #         x_min = self.length_before_control - 1.0/20.0 * self.control_length
    #         x_max = self.tuning_control_width + self.length_before_control
    #         y_height= self. step_height
    #         return on_boundary and x[0] >= x_min and x[0] <= x_max and x[1] == y_height

    # def boundary_bf_control(x, on_boundary): #,control_length,tuning_control_width,length_before_control,step_height):
    #         x_min = 0.0
    #         x_max= self.length_before_control - 1.0/20.0 * self.control_length
    #         y_height = self.step_height
    #         return on_boundary and x[0] >= x_min and x[0] <= x_max and x[1] == y_height 
